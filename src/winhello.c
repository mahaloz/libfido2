    ...
    const char *pin, int ms)
{
	...
	/* options */
	opt = &ctx->opt;
	opt->dwVersion = WEBAUTHN_AUTHENTICATOR_GET_ASSERTION_OPTIONS_VERSION_5;
	opt->dwTimeoutMilliseconds = ms < 0 ? MAXMSEC : (DWORD)ms;
	opt->dwCredLargeBlobOperation = WEBAUTHN_CRED_LARGE_BLOB_OPERATION_SET;
	opt->cbCredLargeBlob = sizeof(random_words);
	opt->pbCredLargeBlob = random_words;
